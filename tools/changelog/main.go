...
func showChangeLog(ctx context.Context, slackChannel string) error {
	...
	releases, _, err := client.Repositories.ListReleases(ctx, TEALBASE_OWNER, TEALBASE_REPO, &github.ListOptions{})
	if err != nil {
		return err
	}
	...
	notes, _, err := client.Repositories.GenerateReleaseNotes(ctx, TEALBASE_OWNER, TEALBASE_REPO, &opts)
	if err != nil {
		return err
	}
