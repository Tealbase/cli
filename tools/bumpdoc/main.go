...
func updateRefDoc(ctx context.Context, path string, stdin io.Reader) error {
	...
	branch := "cli/ref-doc"
	master := "master"
	if err := shared.CreateGitBranch(ctx, client, TEALBASE_OWNER, TEALBASE_REPO, branch, master); err != nil {
		return err
	}
	// Get original file
	opts := github.RepositoryContentGetOptions{Ref: "heads/" + branch}
	file, _, _, err := client.Repositories.GetContents(ctx, TEALBASE_OWNER, TEALBASE_REPO, path, &opts)
	if err != nil {
		return err
	}
	...
	resp, _, err := client.Repositories.UpdateFile(ctx, TEALBASE_OWNER, TEALBASE_REPO, path, &commit)
	if err != nil {
		return err
	}
	...
	return shared.CreatePullRequest(ctx, client, TEALBASE_OWNER, TEALBASE_REPO, pr)
}
