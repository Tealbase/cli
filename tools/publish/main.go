...
func publishPackages(ctx context.Context, version string, beta bool) error {
	...
	config.FormulaName = "Tealbase"
	config.Description = "Tealbase CLI"
	filename := "tealbase"
	if beta {
...
func fetchConfig(ctx context.Context, version string) (PackageConfig, error) {
	...
	url := fmt.Sprintf("https://github.com/tealbase/cli/releases/download/v%[1]v/tealbase_%[1]v_checksums.txt", config.Version)
	log.Println(url)
...
func updatePackage(ctx context.Context, client *github.Client, repo, path string, tmpl *template.Template, config PackageConfig) error {
	...
	branch := "release/cli"
	master := "main"
	if err := shared.CreateGitBranch(ctx, client, TEALBASE_OWNER, repo, branch, master); err != nil {
		return err
	}
	// Get file SHA
	opts := github.RepositoryContentGetOptions{Ref: "heads/" + branch}
	file, _, _, err := client.Repositories.GetContents(ctx, TEALBASE_OWNER, repo, path, &opts)
	if err != nil {
		return err
	}
	...
	resp, _, err := client.Repositories.UpdateFile(ctx, TEALBASE_OWNER, repo, path, &commit)
	if err != nil {
		return err
	}
	...
	return shared.CreatePullRequest(ctx, client, TEALBASE_OWNER, repo, pr)
}
