...
func TestVerifyCNAME(t *testing.T) {
	...
	gock.New("https://1.1.1.1").
		...
		JSON(&map[string]interface{}{"Answer": []map[string]interface{}{
			{
				"Type": 5, "Data": "foobarbaz.tealbase.co.",
			},
		}})
...
func TestVerifyCNAMEFailures(t *testing.T) {
	...
	err := VerifyCNAME(context.Background(), "foobarbaz", "hello.custom-domain.com")
	assert.ErrorContains(t, err, "expected custom hostname 'hello.custom-domain.com' to have a CNAME record pointing to your project at 'foobarbaz.tealbase.co.', but it failed to resolve: failed to locate appropriate CNAME record for hello.custom-domain.com")
}
