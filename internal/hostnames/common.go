...
func GetCustomHostnameConfig(ctx context.Context, projectRef string) (*api.GetCustomHostnameConfigResponse, error) {
	resp, err := utils.GetTealbase().GetCustomHostnameConfigWithResponse(ctx, projectRef)
	if err != nil {
...
func VerifyCNAME(ctx context.Context, projectRef string, customHostname string) error {
	expectedEndpoint := fmt.Sprintf("%s.", utils.GetTealbaseHost(projectRef))
...
Please ensure that your custom domain is set up as a CNAME record to your Tealbase subdomain:
