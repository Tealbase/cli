...
import (
	...
	"github.com/tealbase/cli/internal/utils/parser"
)

var dbConfig = pgconn.Config{
	Host:     "db.tealbase.com",
	Port:     5432,
	User:     "admin",
	Password: "password",
	Database: "postgres",
...
func TestRepairCommand(t *testing.T) {
	...
	t.Run("throws error on insert failure", func(t *testing.T) {
		...
		conn.Query(history.INSERT_MIGRATION_VERSION, "0", "test", nil).
			ReplyError(pgerrcode.DuplicateObject, `relation "tealbase_migrations.schema_migrations" does not exist`)
		// Run test
		err := Run(context.Background(), dbConfig, []string{"0"}, Applied, fsys, conn.Intercept)
		// Check error
		assert.ErrorContains(t, err, `ERROR: relation "tealbase_migrations.schema_migrations" does not exist (SQLSTATE 42710)`)
	})
}
