...
func TestPendingMigrations(t *testing.T) {
	...
	t.Run("throws error on missing local migration", func(t *testing.T) {
		...
		assert.Contains(t, utils.CmdSuggestion, "tealbase migration repair --status reverted 0")
	})
...
func TestIgnoreVersionMismatch(t *testing.T) {
	...
	t.Run("throws error on missing local migration", func(t *testing.T) {
		...
		// Check error
		assert.ErrorIs(t, err, errMissingLocal)
		assert.Contains(t, utils.CmdSuggestion, "tealbase migration repair --status reverted 20221201000001")
	})
}
