...
import (
	...
	"github.com/tealbase/cli/internal/migration/history"
	"github.com/tealbase/cli/internal/migration/repair"
	"github.com/tealbase/cli/internal/testing/apitest"
	"github.com/tealbase/cli/internal/testing/fstest"
	"github.com/tealbase/cli/internal/testing/pgtest"
...
func TestSquashCommand(t *testing.T) {
	...
	t.Run("baselines migration history", func(t *testing.T) {
		...
		defer conn.Close(t)
		pgtest.MockMigrationHistory(conn)
		conn.Query(fmt.Sprintf("DELETE FROM tealbase_migrations.schema_migrations WHERE version <=  '0' ;INSERT INTO tealbase_migrations.schema_migrations(version, name, statements) VALUES( '0' ,  'init' ,  '{%s}' )", sql)).
			Reply("INSERT 0 1")
		// Run test
...
func TestBaselineMigration(t *testing.T) {
	...
	t.Run("throws error on query failure", func(t *testing.T) {
		...
		defer conn.Close(t)
		pgtest.MockMigrationHistory(conn)
		conn.Query(fmt.Sprintf("DELETE FROM tealbase_migrations.schema_migrations WHERE version <=  '%[1]s' ;INSERT INTO tealbase_migrations.schema_migrations(version, name, statements) VALUES( '%[1]s' ,  'init' ,  null )", "0")).
			ReplyError(pgerrcode.InsufficientPrivilege, "permission denied for relation tealbase_migrations")
		// Run test
		err := baselineMigrations(context.Background(), dbConfig, "0", fsys, conn.Intercept, func(cc *pgx.ConnConfig) {
			cc.PreferSimpleProtocol = true
		})
		// Check error
		assert.ErrorContains(t, err, `ERROR: permission denied for relation tealbase_migrations (SQLSTATE 42501)`)
	})
