...
func deployFunction(ctx context.Context, projectRef, slug, entrypointUrl, importMapUrl string, verifyJWT bool, functionBody io.Reader) error {
	resp, err := utils.GetTealbase().GetFunctionWithResponse(ctx, projectRef, slug)
	if err != nil {
	...
	case http.StatusNotFound: // Function doesn't exist yet, so do a POST
		resp, err := utils.GetTealbase().CreateFunctionWithBodyWithResponse(ctx, projectRef, &api.CreateFunctionParams{
			Slug:           &slug,
			Name:           &slug,
			VerifyJwt:      &verifyJWT,
		...
		if resp.JSON201 == nil {
			return errors.New("Failed to create a new Function on the Tealbase project: " + string(resp.Body))
		}
	case http.StatusOK: // Function already exists, so do a PATCH
		resp, err := utils.GetTealbase().UpdateFunctionWithBodyWithResponse(ctx, projectRef, slug, &api.UpdateFunctionParams{
			VerifyJwt:      &verifyJWT,
			ImportMapPath:  &importMapUrl,
			EntrypointPath: &entrypointUrl,
		}, eszipContentType, functionBody)
		...
		if resp.JSON200 == nil {
			return errors.New("Failed to update an existing Function's body on the Tealbase project: " + string(resp.Body))
		}
	default:
	...
	url := fmt.Sprintf("%s/project/%v/functions/%v/details", utils.GetTealbaseDashboardURL(), projectRef, slug)
