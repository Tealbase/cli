...
func TestDeployCommand(t *testing.T) {
	...
	t.Run("throws error on empty functions", func(t *testing.T) {
		...
		// Check error
		assert.ErrorContains(t, err, "No Functions specified or found in tealbase/functions")
	})
	...
	t.Run("verify_jwt param falls back to config", func(t *testing.T) {
		...
		fsys := afero.NewMemMapFs()
		require.NoError(t, utils.WriteConfig(fsys, false))
		f, err := fsys.OpenFile("tealbase/config.toml", os.O_APPEND|os.O_WRONLY, 0600)
		require.NoError(t, err)
		_, err = f.WriteString(`
[functions.` + slug + `]
verify_jwt = false
...
func TestDeployFunction(t *testing.T) {
	...
	t.Run("throws error on create unavailable", func(t *testing.T) {
		...
		err := deployFunction(context.Background(), project, slug, "", "", true, strings.NewReader("body"))
		// Check error
		assert.ErrorContains(t, err, "Failed to create a new Function on the Tealbase project:")
	})
	...
	t.Run("throws error on update unavailable", func(t *testing.T) {
		...
		err := deployFunction(context.Background(), project, slug, "", "", true, strings.NewReader("body"))
		// Check error
		assert.ErrorContains(t, err, "Failed to update an existing Function's body on the Tealbase project:")
	})
}
