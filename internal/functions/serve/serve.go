...
func ServeFunctions(ctx context.Context, envFilePath string, noVerifyJWT *bool, importMapPath string, dbUrl string, w io.Writer, fsys afero.Fs) error {
	...
	env := []string{
		"TEALBASE_URL=http://" + utils.KongAliases[0] + ":8000",
		"TEALBASE_ANON_KEY=" + utils.Config.Auth.AnonKey,
		"TEALBASE_SERVICE_ROLE_KEY=" + utils.Config.Auth.ServiceRoleKey,
		"TEALBASE_DB_URL=" + dbUrl,
		"TEALBASE_INTERNAL_JWT_SECRET=" + utils.Config.Auth.JwtSecret,
		fmt.Sprintf("TEALBASE_INTERNAL_HOST_PORT=%d", utils.Config.Api.Port),
		"TEALBASE_INTERNAL_FUNCTIONS_PATH=" + utils.DockerFuncDirPath,
	}
	if viper.GetBool("DEBUG") {
		env = append(env, "TEALBASE_INTERNAL_DEBUG=true")
	}
	...
	env = append(env, "TEALBASE_INTERNAL_FUNCTIONS_CONFIG="+functionsConfigString)
...
func parseEnvFile(envFilePath string, fsys afero.Fs) ([]string, error) {
	...
	for name, value := range envMap {
		if strings.HasPrefix(name, "TEALBASE_") {
			return env, errors.Errorf("Invalid env name: %s. Env names cannot start with TEALBASE_.", name)
		}
...
func populatePerFunctionConfigs(binds []string, importMapPath string, noVerifyJWT *bool, fsys afero.Fs) ([]string, string, error) {
	...
	for _, function := range functions {
		...
		if importMapPath != "" {
			...
			hostImportMapPath := filepath.Join(cwd, utils.TealbaseDirPath, functionConfig.ImportMap)
