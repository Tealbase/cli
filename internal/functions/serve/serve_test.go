...
func TestServeCommand(t *testing.T) {
	t.Run("serves all functions", func(t *testing.T) {
		...
		gock.New(utils.Docker.DaemonHost()).
			Get("/v" + utils.Docker.ClientVersion() + "/containers/tealbase_db_test/json").
			Reply(http.StatusOK).
			JSON(types.ContainerJSON{})
		containerId := "tealbase_edge_runtime_test"
		gock.New(utils.Docker.DaemonHost()).
	...
	t.Run("throws error on missing config", func(t *testing.T) {
		...
		// Check error
		assert.ErrorContains(t, err, "open tealbase/config.toml: file does not exist")
	})
