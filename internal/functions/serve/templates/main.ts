...
const JWT_SECRET = Deno.env.get("TEALBASE_INTERNAL_JWT_SECRET")!;
const HOST_PORT = Deno.env.get("TEALBASE_INTERNAL_HOST_PORT")!;
// OS stuff - we don't want to expose these to the functions.
const EXCLUDED_ENVS = ["HOME", "HOSTNAME", "PATH", "PWD"];
const FUNCTIONS_PATH = Deno.env.get("TEALBASE_INTERNAL_FUNCTIONS_PATH")!;
const DEBUG = Deno.env.get("TEALBASE_INTERNAL_DEBUG") === "true";
const FUNCTIONS_CONFIG_STRING = Deno.env.get(
  "TEALBASE_INTERNAL_FUNCTIONS_CONFIG",
)!;
...
serve(async (req: Request) => {
  ...
  const envVars = Object.entries(envVarsObj)
    .filter(([name, _]) =>
      !EXCLUDED_ENVS.includes(name) && !name.startsWith("TEALBASE_INTERNAL_")
    );
