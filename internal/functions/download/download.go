...
func getFunctionMetadata(ctx context.Context, projectRef, slug string) (*api.FunctionSlugResponse, error) {
	...
	case http.StatusNotFound:
		return nil, errors.Errorf("Function %s does not exist on the Tealbase project.", utils.Aqua(slug))
	case http.StatusOK:
		break
	default:
		return nil, errors.Errorf("Failed to download Function %s on the Tealbase project: %s", utils.Aqua(slug), string(resp.Body))
	}
...
func downloadFunction(ctx context.Context, projectRef, slug, extractScriptPath string) error {
	...
	resp, err := utils.GetTealbase().GetFunctionBodyWithResponse(ctx, projectRef, slug)
	if err != nil {
...
func suggestLegacyBundle(slug string) string {
	return fmt.Sprintf("\nIf your function is deployed using CLI < 1.120.0, trying running %s instead.", utils.Aqua("tealbase functions download --legacy-bundle "+slug))
}
