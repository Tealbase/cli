...
import (
	...
	"github.com/tealbase/cli/internal/db/reset"
	"github.com/tealbase/cli/internal/db/start"
	"github.com/tealbase/cli/internal/functions/serve"
...
func suggestUpdateCmd(serviceImages map[string]string) string {
	...
	cmd += fmt.Sprintf("Run %s to update them.", utils.Aqua("tealbase link"))
	return cmd
}
...
func Run(ctx context.Context, fsys afero.Fs, excludedContainers []string, ignoreHealthCheck bool) error {
	// Sanity checks.
	{
		...
		if err := utils.AssertTealbaseDbIsRunning(); err == nil {
			fmt.Fprintln(os.Stderr, utils.Aqua("tealbase start")+" is already running.")
			utils.CmdSuggestion = fmt.Sprintf("Run %s to show status of local Tealbase containers.", utils.Aqua("tealbase status"))
			return nil
...
`},
	...
	if utils.Config.Analytics.Enabled && !isContainerExcluded(utils.LogflareImage, excluded) {
		env := []string{
			...
			"DB_USERNAME=tealbase_admin",
			"DB_PASSWORD=" + dbConfig.Password,
			"LOGFLARE_MIN_CLUSTER_SIZE=1",
			"LOGFLARE_SINGLE_TENANT=true",
			"LOGFLARE_TEALBASE_MODE=true",
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					"KONG_DNS_ORDER=LAST,A,CNAME", // https://github.com/tealbase/cli/issues/14
...
`},
	...
	if utils.Config.Auth.Enabled && !isContainerExcluded(utils.Config.Auth.Image, excluded) {
		...
		env := []string{
			...
			fmt.Sprintf("GOTRUE_DB_DATABASE_URL=postgresql://tealbase_auth_admin:%s@%s:%d/%s", dbConfig.Password, dbConfig.Host, dbConfig.Port, dbConfig.Database),
			...
			// https://github.com/tealbase/tealbase/issues/16100
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					"PGRST_JWT_SECRET=" + utils.Config.Auth.JwtSecret,
					fmt.Sprintf("DATABASE_URL=postgresql://tealbase_storage_admin:%s@%s:%d/%s", dbConfig.Password, dbConfig.Host, dbConfig.Port, dbConfig.Database),
					fmt.Sprintf("FILE_SIZE_LIMIT=%v", utils.Config.Storage.FileSizeLimit),
					"STORAGE_BACKEND=file",
					"FILE_STORAGE_BACKEND_PATH=" + dockerStoragePath,
					"TENANT_ID=stub",
					// TODO: https://github.com/tealbase/storage-api/issues/55
					"REGION=stub",
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					"TEALBASE_URL=http://" + utils.KongId + ":8000",
					fmt.Sprintf("TEALBASE_PUBLIC_URL=%s:%v/", utils.Config.Studio.ApiUrl, utils.Config.Api.Port),
					"TEALBASE_ANON_KEY=" + utils.Config.Auth.AnonKey,
					"TEALBASE_SERVICE_KEY=" + utils.Config.Auth.ServiceRoleKey,
					"LOGFLARE_API_KEY=" + utils.Config.Analytics.ApiKey,
		...
		if _, err := utils.DockerStart(
			...
			container.Config{
				...
				Env: []string{
					...
					// Default platform config: https://github.com/tealbase/postgres/blob/develop/ansible/files/pgbouncer_config/pgbouncer.ini.j2
