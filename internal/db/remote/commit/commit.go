...
func Run(ctx context.Context, schema []string, config pgconn.Config, fsys afero.Fs) error {
	...
	fmt.Println("Finished " + utils.Aqua("tealbase db remote commit") + ".")
	return nil
}
...
- Pushing unapplied migrations with ` + utils.Aqua("tealbase db push") + `,
- Or failing that, manually editing tealbase_migrations.schema_migrations table with ` + utils.Aqua("tealbase migration repair") + ".")
