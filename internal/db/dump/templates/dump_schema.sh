...
#
#   - do not alter superuser role "tealbase_admin"
...
#   - do not include creating publication "tealbase_realtime"
pg_dump \
    --schema-only \
...
| sed -E 's/^CREATE PUBLICATION "tealbase_realtime"/-- &/' \
| sed -E 's/^CREATE EVENT TRIGGER /-- &/' \
| sed -E 's/^         WHEN TAG IN /-- &/' \
| sed -E 's/^   EXECUTE FUNCTION /-- &/' \
| sed -E 's/^ALTER EVENT TRIGGER /-- &/' \
| sed -E 's/^ALTER DEFAULT PRIVILEGES FOR ROLE "tealbase_admin"/-- &/' \
