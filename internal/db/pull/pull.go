...
import (
	...
	"github.com/tealbase/cli/internal/migration/new"
...
var (
	...
	suggestExtraPull = fmt.Sprintf(
		...
		utils.Bold("auth"),
		utils.Bold("storage"),
		utils.Aqua("tealbase db pull --schema auth,storage"),
	)
)
...
func run(p utils.Program, ctx context.Context, schema []string, path string, conn *pgx.Conn, fsys afero.Fs) error {
	...
	// 1. Assert `tealbase/migrations` and `schema_migrations` are in sync.
...
func suggestMigrationRepair(extraRemote, extraLocal []string) string {
	...
	for _, version := range extraRemote {
		result += fmt.Sprintln(utils.Bold("tealbase migration repair --status reverted " + version))
	}
	for _, version := range extraLocal {
		result += fmt.Sprintln(utils.Bold("tealbase migration repair --status applied " + version))
	}
	return result
}
