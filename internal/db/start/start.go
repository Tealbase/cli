...
func initSchema15(ctx context.Context, host string) error {
	...
	if err := utils.DockerRunOnceWithStream(ctx, utils.Config.Storage.Image, []string{
		...
		fmt.Sprintf("DATABASE_URL=postgresql://tealbase_storage_admin:%s@%s:5432/postgres", utils.Config.Db.Password, host),
	...
	return utils.DockerRunOnceWithStream(ctx, utils.Config.Auth.Image, []string{
		...
		fmt.Sprintf("GOTRUE_DB_DATABASE_URL=postgresql://tealbase_auth_admin:%s@%s:5432/postgres", utils.Config.Db.Password, host),
