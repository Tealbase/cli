...
import (
	...
	"github.com/tealbase/cli/internal/gen/keys"
	"github.com/tealbase/cli/internal/migration/apply"
	...
	"github.com/tealbase/cli/internal/utils/pgxv5"
)
...
func Run(ctx context.Context, version string, config pgconn.Config, fsys afero.Fs, options ...func(*pgx.ConnConfig)) error {
	...
	fmt.Fprintln(os.Stderr, "Finished "+utils.Aqua("tealbase db reset")+" on branch "+utils.Aqua(branch)+".")
	return nil
}
...
func initDatabase(ctx context.Context, options ...func(*pgx.ConnConfig)) error {
	conn, err := utils.ConnectLocalPostgres(ctx, pgconn.Config{User: "tealbase_admin"}, options...)
	if err != nil {
		return err
	}
	defer conn.Close(context.Background())
...
func recreateDatabase(ctx context.Context, options ...func(*pgx.ConnConfig)) error {
	conn, err := utils.ConnectLocalPostgres(ctx, pgconn.Config{User: "tealbase_admin", Database: "template1"}, options...)
	if err != nil {
		return err
	}
	defer conn.Close(context.Background())
