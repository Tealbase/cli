...
type CustomName struct {
	DbHost         string `env:"db.host,default=NEXT_PUBLIC_TEALBASE_URL"`
	DbPassword     string `env:"db.password,default=TEALBASE_DB_PASSWORD"`
	JWTSecret      string `env:"db.password,default=TEALBASE_AUTH_JWT_SECRET"`
	AnonKey        string `env:"auth.anon_key,default=TEALBASE_AUTH_ANON_KEY"`
	ServiceRoleKey string `env:"auth.service_role_key,default=TEALBASE_AUTH_SERVICE_ROLE_KEY"`
}
...
func GenerateSecrets(ctx context.Context, projectRef, branch string, fsys afero.Fs) error {
	...
	resp, err := utils.GetTealbase().GetPostgRESTConfigWithResponse(ctx, projectRef)
	if err != nil {
