...
func TestStatusCommand(t *testing.T) {
	t.Run("shows service status", func(t *testing.T) {
		services := []string{
			...
			"tealbase_auth_",
			"tealbase_inbucket_",
			"realtime-dev.tealbase_realtime_",
			"tealbase_rest_",
			"tealbase_storage_",
			"storage_imgproxy_",
			"tealbase_pg_meta_",
	...
	t.Run("throws error on missing docker", func(t *testing.T) {
		...
		defer gock.OffAll()
		gock.New(utils.Docker.DaemonHost()).
			Get("/v" + utils.Docker.ClientVersion() + "/containers/tealbase_db_").
...
func TestServiceHealth(t *testing.T) {
	services := []string{"tealbase_db", "tealbase_auth"}
	...
	t.Run("checks all services", func(t *testing.T) {
		...
		assert.ElementsMatch(t, []string{
			"tealbase_db container is not ready: Unhealthy",
			"tealbase_auth container is not running: exited",
		}, lines)
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
