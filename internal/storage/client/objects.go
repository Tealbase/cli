...
import (
	...
	"github.com/tealbase/cli/internal/utils/tenant"
)
...
func ListStorageObjects(ctx context.Context, projectRef, bucket, prefix string, page int) ([]ObjectResponse, error) {
	url := fmt.Sprintf("https://%s/storage/v1/object/list/%s", utils.GetTealbaseHost(projectRef), bucket)
...
func UploadStorageObject(ctx context.Context, projectRef, remotePath, localPath string, fsys afero.Fs, opts ...func(*FileOptions)) error {
	...
	// Use default value of storage-js: https://github.com/tealbase/storage-js/blob/main/src/packages/StorageFileApi.ts#L22
	...
	url := fmt.Sprintf("https://%s/storage/v1/object/%s", utils.GetTealbaseHost(projectRef), remotePath)
...
func MoveStorageObject(ctx context.Context, projectRef, bucketId, srcPath, dstPath string) (*MoveObjectResponse, error) {
	url := fmt.Sprintf("https://%s/storage/v1/object/move", utils.GetTealbaseHost(projectRef))
	apiKey, err := tenant.GetApiKeys(ctx, projectRef)
	if err != nil {
		return nil, err
	}
...
func CopyStorageObject(ctx context.Context, projectRef, bucketId, srcPath, dstPath string) (*CopyObjectResponse, error) {
	url := fmt.Sprintf("https://%s/storage/v1/object/copy", utils.GetTealbaseHost(projectRef))
	apiKey, err := tenant.GetApiKeys(ctx, projectRef)
	if err != nil {
		return nil, err
	}
...
func DeleteStorageObjects(ctx context.Context, projectRef, bucket string, prefixes []string) ([]DeleteObjectsResponse, error) {
	url := fmt.Sprintf("https://%s/storage/v1/object/%s", utils.GetTealbaseHost(projectRef), bucket)
	apiKey, err := tenant.GetApiKeys(ctx, projectRef)
	if err != nil {
		return nil, err
	}
