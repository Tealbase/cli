...
import (
	...
	"github.com/tealbase/cli/internal/utils/credentials"
)
...
func Run(ctx context.Context, stdout *os.File, params RunParams) error {
	...
	if params.OpenBrowser {
		fmt.Fprint(stdout, "Hello from ", utils.Aqua("Tealbase"), "! Press ", utils.Aqua("Enter"), " to open browser and login automatically.\n")
		fmt.Scanln()
	}
	...
	createLoginSessionUrl := utils.GetTealbaseDashboardURL() + createLoginSessionPath + createLoginSessionQuery
	...
	err := utils.RunProgram(ctx, func(p utils.Program, ctx context.Context) error {
		...
		sessionPollingUrl := utils.GetTealbaseAPIHost() + "/platform/cli/login/" + params.SessionId
...
Enter your access token: `, utils.GetTealbaseDashboardURL())
