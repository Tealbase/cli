...
import (
	...
	"github.com/tealbase/cli/internal/debug"
)
...
func GetPoolerConfig(projectRef string) *pgconn.Config {
	...
	if !isTealbaseDomain(poolerConfig.Host) {
		fmt.Fprintln(logger, "Pooler hostname does not belong to Tealbase domain:", poolerConfig.Host)
		return nil
	}
...
func isTealbaseDomain(host string) bool {
	switch GetTealbaseAPIHost() {
	case "https://api.tealbase.green":
		return strings.HasSuffix(host, ".tealbase.green")
	default:
		return strings.HasSuffix(host, ".tealbase.com")
	}
}
