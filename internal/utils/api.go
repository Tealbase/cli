...
import (
	...
	tealbase "github.com/tealbase/cli/pkg/api"
)
...
var (
	...
	apiClient  *tealbase.ClientWithResponses
...
func GetTealbase() *tealbase.ClientWithResponses {
	clientOnce.Do(func() {
		...
		apiClient, err = tealbase.NewClientWithResponses(
			GetTealbaseAPIHost(),
			tealbase.WithRequestEditorFn(func(ctx context.Context, req *http.Request) error {
				req.Header.Set("Authorization", "Bearer "+token)
				req.Header.Set("User-Agent", "TealbaseCLI/"+Version)
				return nil
			}),
		)
		if err != nil {
...
const (
	DefaultApiHost = "https://api.tealbase.com"
	// DEPRECATED
	DeprecatedApiHost = "https://api.tealbase.io"
)
...
func GetTealbaseAPIHost() string {
...
func GetTealbaseDashboardURL() string {
	switch GetTealbaseAPIHost() {
	case DefaultApiHost, DeprecatedApiHost:
		return "https://tealbase.com/dashboard"
	case "https://api.tealbase.green":
		return "https://app.tealbase.green"
	default:
		return "http://127.0.0.1:8082"
	}
}

func GetTealbaseDbHost(projectRef string) string {
	// TODO: query projects api for db_host
	switch GetTealbaseAPIHost() {
	case DefaultApiHost, DeprecatedApiHost:
		return fmt.Sprintf("db.%s.tealbase.co", projectRef)
	case "https://api.tealbase.green":
		return fmt.Sprintf("db.%s.tealbase.red", projectRef)
	default:
		return fmt.Sprintf("db.%s.tealbase.red", projectRef)
	}
}

func GetTealbaseHost(projectRef string) string {
	switch GetTealbaseAPIHost() {
	case DefaultApiHost, DeprecatedApiHost:
		return fmt.Sprintf("%s.tealbase.co", projectRef)
	case "https://api.tealbase.green":
		return fmt.Sprintf("%s.tealbase.red", projectRef)
	default:
		return fmt.Sprintf("%s.tealbase.red", projectRef)
	}
}
