...
var (
	...
	DbAliases          = []string{"db", "db.tealbase.internal"}
	KongAliases        = []string{"kong", "api.tealbase.internal"}
...
func GetId(name string) string {
	return "tealbase_" + name + "_" + Config.ProjectId
}
...
//
// > secret = "env(TEALBASE_AUTH_EXTERNAL_APPLE_SECRET)"
//
...
// variable, such as TEALBASE_AUTH_ANON_KEY.
//
...
func LoadConfigFS(fsys afero.Fs) error {
	...
	if metadata, err := toml.DecodeFS(afero.NewIOFS(fsys), ConfigPath, &Config); err != nil {
		CmdSuggestion = fmt.Sprintf("Have you set up the project with %s?", Aqua("tealbase init"))
		cwd, osErr := os.Getwd()
		if osErr != nil {
	...
	// Process decoded TOML.
	{
		...
		case 12:
			return errors.New("Postgres version 12.x is unsupported. To use the CLI, either start a new project or follow project migration steps here: https://tealbase.com/docs/guides/database#migrating-between-projects.")
		case 13:
		...
		case 15:
			if len(Config.Experimental.OrioleDBVersion) > 0 {
				Config.Db.Image = "tealbase/postgres:orioledb-" + Config.Experimental.OrioleDBVersion
				var err error
