...
func TestLookupIP(t *testing.T) {
	...
	t.Run("resolves IPv6 recursively", func(t *testing.T) {
		...
		gock.New("https://1.1.1.1").
			Get("/dns-query").
			MatchParam("name", "api.tealbase.com").
			MatchHeader("accept", "application/dns-json").
			Reply(http.StatusOK).
			JSON(&dnsResponse{Answer: []dnsAnswer{
				{Type: 5, Data: "tealbase-api.fly.dev."},
				{Type: dnsIPv6Type, Data: "2606:2800:220:1:248:1893:25c8:1946"},
			}})
		// Run test
		ip, err := FallbackLookupIP(context.Background(), "api.tealbase.com")
		// Validate output
		assert.NoError(t, err)
	...
	t.Run("empty on no answer", func(t *testing.T) {
		...
		ip, err := FallbackLookupIP(context.Background(), host)
		// Validate output
		assert.ErrorContains(t, err, "failed to locate valid IP for api.tealbase.io; resolves to []utils.dnsAnswer(nil)")
		assert.Empty(t, ip)
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
}
...
func TestResolveCNAME(t *testing.T) {
	t.Run("resolves CNAMEs with CloudFlare", func(t *testing.T) {
		...
		gock.New("https://1.1.1.1").
			...
			Reply(http.StatusOK).
			JSON(&dnsResponse{Answer: []dnsAnswer{
				{Type: cnameType, Data: "foobarbaz.tealbase.co"},
			}})
		// Run test
		cname, err := ResolveCNAME(context.Background(), host)
		// Validate output
		assert.Equal(t, "foobarbaz.tealbase.co", cname)
		assert.Nil(t, err)
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
	...
	t.Run("missing CNAMEs return an error", func(t *testing.T) {
		...
		assert.ErrorContains(t, err, "failed to locate appropriate CNAME record for api.tealbase.io")
		assert.Empty(t, apitest.ListUnmatchedRequests())
	})
