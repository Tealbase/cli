...
const (
	CliProjectLabel     = "com.tealbase.cli.project"
...
// Defaults to Tealbase public ECR for faster image pull
...
func GetRegistryImageUrl(imageName string) string {
	...
	return registry + "/tealbase/" + imageName
}
			...
			for _, p := range c.Ports {
				if fmt.Sprintf("%s:%d", p.IP, p.PublicPort) == hostPort {
					if project, ok := c.Labels[CliProjectLabel]; ok {
						return "\nTry stopping the running project with " + Aqua("tealbase stop --project-id "+project)
					} else {
						name := c.ID
						if len(c.Names) > 0 {
							name = c.Names[0]
