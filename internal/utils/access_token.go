...
var (
	...
	ErrMissingToken    = errors.Errorf("Access token not provided. Supply an access token by running %s or setting the TEALBASE_ACCESS_TOKEN environment variable.", Aqua("tealbase login"))
...
func loadAccessToken(fsys afero.Fs) (string, error) {
	...
	if accessToken := os.Getenv("TEALBASE_ACCESS_TOKEN"); accessToken != "" {
		return accessToken, nil
	}
...
func getAccessTokenPath() (string, error) {
	...
	return filepath.Join(home, ".tealbase", AccessTokenKey), nil
}
