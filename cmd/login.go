...
import (
	...
	"github.com/tealbase/cli/internal/login"
	"github.com/tealbase/cli/internal/utils"
	"golang.org/x/term"
)

var (
	ErrMissingToken = errors.Errorf("Cannot use automatic login flow inside non-TTY environments. Please provide %s flag or set the %s environment variable.", utils.Aqua("--token"), utils.Aqua("TEALBASE_ACCESS_TOKEN"))
)
	...
	loginCmd = &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !term.IsTerminal(int(os.Stdin.Fd())) {
				...
				params.Token = token
			} else if token := os.Getenv("TEALBASE_ACCESS_TOKEN"); token != "" {
				params.Token = token
			}
