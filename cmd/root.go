...
func promptLogin(fsys afero.Fs) error {
	if _, err := utils.LoadAccessTokenFS(fsys); err == utils.ErrMissingToken {
		utils.CmdSuggestion = fmt.Sprintf("Run %s first.", utils.Aqua("tealbase login"))
...
var (
	...
	rootCmd = &cobra.Command{
		Use:     "tealbase",
		Short:   "Tealbase CLI " + utils.Version,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			apiHost, err := url.Parse(utils.GetTealbaseAPIHost())
			if err != nil {
				return err
			}
...
func recoverAndExit() {
	...
	if createTicket && len(utils.SentryDsn) > 0 {
		...
		if eventId != nil && sentry.Flush(2*time.Second) {
			...
			fmt.Fprintln(os.Stderr, "Quote the crash ID above when filing a bug report: https://github.com/tealbase/cli/issues/new/choose")
		}
	}
	os.Exit(1)
}
...
func init() {
	...
	flags.String("workdir", "", "path to a Tealbase project directory")
