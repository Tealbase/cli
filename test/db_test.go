...
func (suite *DBTestSuite) SetupTest() {
	...
	if err != nil {
		suite.Fail("failed to init tealbase cli")
	}

	err = os.Mkdir("tealbase/.branches", os.FileMode(0755))
	if err != nil {
		suite.Fail("failed to create tealbase/.branches directory")
	}
}
...
func (suite *DBTestSuite) constructParams() []gin.Params {
	...
	for _, id := range suite.ids {
		...
		ids = append(ids, gin.Params{
			gin.Param{
				Key:   "id",
				Value: "tealbase_db_" + filepath.Base(suite.tempDir),
			},
		})
