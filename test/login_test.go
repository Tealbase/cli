...
func (suite *LoginTestSuite) TestLink() {
	...
	key := "sbp_" + gonanoid.MustGenerate(tealbase.KeyAlphabet, tealbase.KeyLength)
	...
	require.NoError(suite.T(), err)
	_, err = os.Stat(filepath.Join(home, ".tealbase/access-token"))
	require.NoError(suite.T(), err)
	token, err := os.ReadFile(filepath.Join(home, ".tealbase/access-token"))
	require.NoError(suite.T(), err)
