...
func (suite *SecretsTestSuite) TestList() {
	...
	flags.ProjectRef = gonanoid.MustGenerate(tealbase.IDAlphabet, tealbase.IDLength)
	...
	require.Contains(suite.T(), suite.headers, http.Header{
		"Authorization":   []string{fmt.Sprintf("Bearer %s", tealbase.AccessToken)},
		"Accept-Encoding": []string{"gzip"},
		"User-Agent":      []string{"TealbaseCLI/"},
	})
...
func (suite *SecretsTestSuite) SetupTest() {
	...
	// add `link` dir
	require.NoError(suite.T(), os.MkdirAll("tealbase/.temp", os.FileMode(0755)))
